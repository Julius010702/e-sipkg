...
export async function GET(req: Request) {
  try {
    ...
    // Jika ADMIN_SEKOLAH — filter hanya jabatan milik sekolahnya
    // PERBAIKAN: gunakan nested `is` agar Prisma membaca relasi dengan benar
    if (payload.role === "ADMIN_SEKOLAH") {
      where.jabatan = {
        is: { sekolahId: payload.sekolahId },
      };
    }

    // Jika ada filter jabatanId
    if (jabatanId) {
  ...
  try {
    ...
    const pemangku = await prisma.pemangku.create({
      data: {
        jabatanId,
        namaPemangku,
        // PERBAIKAN: nip kosong ("") diubah jadi null agar tidak melanggar unique constraint
        nip: nip?.trim() || null,
        statusPegawai,
...
export async function PUT(req: Request) {
  try {
    ...
    // PERBAIKAN: normalisasi nip kosong → null sebelum update
    if ("nip" in updateData) {
      updateData.nip = updateData.nip?.trim() || null;
    }
